...
//! - The number of input streams to prefetch is a constructor parameter
//!   (default [`DEFAULT_NUM_PREFETCH_INPUT_STREAMS`]) instead of the PR's
//!   DataFusion config option
//!   `optimizer.progressive_eval_num_prefetch_input_streams`, which cannot be
//!   added from outside DataFusion.
...
/// Default number of input streams to prefetch (the stream being polled plus
/// one read-ahead stream). This mitigates the sequential-I/O trade-off of
/// polling inputs one at a time: while stream *i* is being drained, stream
/// *i+1* is already executing.
pub(crate) const DEFAULT_NUM_PREFETCH_INPUT_STREAMS: usize = 2;
...
pub(crate) struct ProgressiveEvalExec {
    ...
    /// Number of input streams to prefetch (current stream included)
    num_prefetch_input_streams: usize,
...
impl ProgressiveEvalExec {
    ...
    pub fn new(
        ...
        Self {
            ...
            fetch,
            num_prefetch_input_streams: DEFAULT_NUM_PREFETCH_INPUT_STREAMS,
            cache,
        }
    }

    /// Set the number of input streams to prefetch (clamped to at least 1,
    /// since the stream currently being polled must be executed).
    #[allow(dead_code)]
    pub fn with_num_prefetch_input_streams(mut self, num: usize) -> Self {
        self.num_prefetch_input_streams = num.max(1);
        self
    }
...
impl ExecutionPlan for ProgressiveEvalExec {
    ...
    fn with_new_children(
        ...
        Ok(Arc::new(
            ...
            .with_num_prefetch_input_streams(self.num_prefetch_input_streams),
        ))
    }
    ...
    fn execute(
        ...
        let result = ProgressiveEvalStream::new(
            ...
            self.fetch,
            self.num_prefetch_input_streams,
        )?;
...
impl ProgressiveEvalStream {
    fn new(
        ...
        // NOTE: unlike the PR this is vendored from, the prefetch count is
        // never widened to all input streams when `fetch` is `None` — lazy
        // execution of input partitions is what bounds memory and time to
        // first row (see the module-level vendoring note).
