...
impl LogicalFileView {
    ...
    ///
    /// Tags are free-form `Map<String, String>` metadata on Add actions. The
    /// kernel materializes them into the scan-row batches under
    /// `fileConstantValues.tags`; `None` means the file carries no tags (or
    /// the column is absent from this batch).
...
mod tests {
    ...
    fn logical_file_view_reads_tags_from_file_constant_values() {
        ...
        let children: Vec<ArrayRef> = fcv_fields
            ...
            .map(|field| match field.name().as_str() {
                "tags" => {
                    ...
                    tags.keys().append_value("sort.bounds");
                    tags.values().append_value("v1");
        ...
        assert_eq!(tags.get("sort.bounds"), Some(&Some("v1".to_string())));
