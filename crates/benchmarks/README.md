...
(`with_file_sort_order` / `with_inferred_file_sort_order`, see `plan.md`):
`sort-gen` generates a Delta table whose files are all sorted by a `timestamp`
column, and `sort-bench` measures `ORDER BY timestamp` streaming queries over
it with different provider configurations.
